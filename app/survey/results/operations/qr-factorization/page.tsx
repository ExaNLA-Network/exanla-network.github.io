...
interface QRFactorizationResponse {
  application: string;
  useCase: string;
  displayName: string;
  responses: {
...
// Categorized questions for QR Factorization
const qrQuestionCategories = [
  {
    title: "Matrix Properties",
    description: "Structural and mathematical properties of matrices for QR factorization",
    questionGroups: [
      {
        title: "Matrix Properties",
        questions: [
          { id: 'qr-matrix-structure', title: 'Matrix Structure', description: 'Structure and properties of matrices' },
          { id: 'qr-matrix-distribution', title: 'Matrix Distribution', description: 'How matrices are distributed across processes/nodes' },
          { id: 'qr-matrix-format', title: 'Matrix Storage Format', description: 'Storage formats used for matrices' },
          { id: 'qr-condition', title: 'Condition Number', description: 'Typical condition number of matrices' },
          { id: 'qr-scale-size', title: 'Matrix Dimensions', description: 'Typical matrix dimensions' },
          { id: 'qr-aspect-ratio', title: 'Matrix Shape', description: 'Typical shape of matrices (m rows × n columns)' }
        ]
      }
    ]
  },
  {
    ...
    description: "What needs to be computed and how Q factor is handled",
    questionGroups: [
      {
        title: "Computation Requirements",
        questions: [
          { id: 'qr-compute-mode', title: 'What to Compute', description: 'What needs to be computed (Q, R, or both)' },
          { id: 'qr-q-handling', title: 'Q Factor Handling', description: 'How the Q factor is needed' }
        ]
      }
    ]
  },
  {
    ...
    description: "Numerical precision and accuracy requirements for QR factorization",
    questionGroups: [
      {
        title: "Accuracy Requirements",
        questions: [
          { id: 'qr-orthogonality', title: 'Q Orthogonality', description: 'Required level of orthogonality for Q (||Q^T Q - I||)' },
          { id: 'qr-factorization-accuracy', title: 'Factorization Accuracy', description: 'Required accuracy for the factorization (||A - QR||/||A||)' },
          { id: 'qr-precision-type', title: 'Working Precision', description: 'Numerical precision used or needed' }
        ]
      }
    ]
  },
  {
    ...
    questions: [
      { id: 'qr-computation-type', title: 'Computation Pattern', description: 'How QR factorizations are typically performed' }
    ]
  },
  {
    ...
    questionGroups: [
      {
        title: "Libraries Used/Interested",
        questions: [
          { id: 'qr-nla-group-distributed-used', title: 'Distributed Dense Libraries (Used)', description: 'Currently used distributed-memory dense linear algebra libraries' },
          { id: 'qr-nla-group-distributed-interested', title: 'Distributed Dense Libraries (Interested)', description: 'Distributed-memory libraries of interest' },
          { id: 'qr-nla-group-specialized-used', title: 'Specialized Libraries (Used)', description: 'Currently used specialized libraries for sparse/structured matrices' },
          { id: 'qr-nla-group-specialized-interested', title: 'Specialized Libraries (Interested)', description: 'Specialized libraries of interest' }
        ]
      }
    ]
  },
    ...
    questionGroups: [
      {
        title: "Benchmarks",
        questions: [
          { id: 'qr-scaling-requirements', title: 'Scaling Requirements', description: 'Parallel scaling requirements' },
          { id: 'qr-input-data-type', title: 'Input Data Type for Benchmarking', description: 'Type of input data used for benchmarking' },
          { id: 'qr-data-provision', title: 'Possibility of Providing Data or Mini-apps', description: 'Type of data provided for benchmarking' }
        ]
      }
    ]
  }
...
export default function QRFactorizationPage() {
  const [responses, setResponses] = useState<QRFactorizationResponse[]>([]);
  ...
  const loadSurveyData = async () => {
    try {
      setLoading(true);
      const surveyFiles = [
        'ntchem-1.json', 'ntchem-2.json', 'quantum-espresso.json'
      ];

      const loadedResponses: QRFactorizationResponse[] = [];
      ...
      for (const file of surveyFiles) {
        try {
          ...
          if (response.ok) {
            ...
            // Only include if qr-factorization is true
            if (data['qr-factorization'] === true) {
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="mt-4 text-gray-600">Loading QR Factorization analysis...</p>
        </div>
      </div>
    );
  }
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="text-center mb-12">
          ...
          <p className="text-xl text-gray-600 max-w-3xl mx-auto">
            Detailed analysis of QR factorization operations 
        ...
        <div className="bg-white rounded-lg shadow mb-8">
          <div className="px-6 py-4 border-b border-gray-200">
            <h2 className="text-2xl font-semibold text-gray-900">Applications Using QR Factorization</h2>
            <p className="text-gray-600">Applications that reported using QR factorization</p>
          </div>
          <div className="p-6">
        ...
        <div className="space-y-12">
          {qrQuestionCategories.map((category, categoryIndex) => (
